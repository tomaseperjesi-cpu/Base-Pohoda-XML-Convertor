...
def get_invoice_list(file_bytes):
    """Vytiahne zoznam faktúr a textu z prvej položky pre interaktívny výber VF (Zoradené)."""
    try:
        ...
        # Zoradenie zoznamu pre užívateľské rozhranie (číselne podľa ID)
        def sort_inv(inv):
            match = re.search(r"(\d+)$", inv['id'])
            return int(match.group(1)) if match else 0
            
        invoices.sort(key=sort_inv)
        return invoices
    except:
        return []
...
def transform_xml(file_bytes, rada, due_days, bank_ids, bank_acc, bank_code, payment_type, sym_const):
    ...
    # Načítanie všetkých položiek zo zdrojového XML
    items_to_process = root.findall('dat:dataPackItem', NS)

    # --- ZORADENIE FAKTÚR (IBA PRE RADU VF) ---
    if rada == 'VF':
        def extract_sort_key(item):
            inv = item.find('inv:invoice', NS)
            if inv is not None:
                hdr = inv.find('inv:invoiceHeader', NS)
                if hdr is not None:
                    n_el = hdr.find('inv:number/typ:numberRequested', NS)
                    if n_el is not None and n_el.text:
                        match = re.search(r"(\d+)$", n_el.text.strip())
                        if match:
                            return int(match.group(1))
            return 0
        items_to_process.sort(key=extract_sort_key)
    # ------------------------------------------
    ...
    for i, item in enumerate(items_to_process, 1):
        ...
        if rada == 'VF':
            # BEZPEČNÁ KONTROLA CEZ SESSION STATE (Tovar = True, Služba = False)
...
if u_file is not None:
    ...
    if rada_sel == 'VF':
        ...
        st.info("Zvoľte, či položky na faktúre predstavujú Služby alebo Tovar. Predvolené sú 'Služby'.")
        inv_list = get_invoice_list(io.BytesIO(content))
        
        # Zobrazenie v stĺpcoch pre prehľadnosť
        for entry in inv_list:
            col1, col2 = st.columns([3, 2])
            with col1:
                st.write(f"**FA {entry['id']}** | {entry['text']}")
            with col2:
                # Nastavenie predvoleného indexu pre Radio (0 = Služby, 1 = Tovar)
                is_tovar = st.session_state.vf_goods_map.get(entry['id'], False)
                choice = st.radio(
                    "Typ:",
                    options=['Služby', 'Tovar'],
                    index=1 if is_tovar else 0,
                    key=f"radio_{entry['id']}",
                    horizontal=True,
                    label_visibility="collapsed"
                )
                # Uložíme do pamäte True, ak zvolil 'Tovar', inak False
                st.session_state.vf_goods_map[entry['id']] = (choice == 'Tovar')
        st.divider()
